...
with open('description_list.json') as data_file:
...
# featureList = []
processed_data = []
# if(debug_mode ==1):    print(inpTweets)
if(debug_mode ==1):    print('Processing Text')
...
def create_sparse_matrix(dic, processed_data_array, granularity, threshold, filename):
    ...
    for i in range(len(processed_data_array)):
        ...
        s += "\n"
        f.write(s)
    if(debug_mode ==1):    print "valid feature: " + str(count)
...
if(debug_mode ==1):    print("Input file has been created...")



# import sklearn.svm.libsvm as svm
from svmutil import *
y, x = svm_read_problem('raw_label_feature')
if(debug_mode ==1):    print("Problem has been read...")

# m = svm_train(y[fr:to], x[fr:to], '-s 1 -h 0 -m 1000')
# mt = svm_train(y, x, '-s 0 -h 0 -m 1000')
# if(debug_mode ==1):    print("Model get trained successfully...")
...
if(debug_mode ==1):    print(p_acc)
for i in range(0, len(p_label)):
    if(debug_mode ==1):    print(i, p_label[i], p_val[i])
    if(debug_mode ==1):    print('\n')

cnt = []
# Ignore the noise class '5'
for i in range(4):
    cnt.append(p_label.count(i+1))
if(debug_mode ==1):    print(cnt)
...
ind = cnt.index(max(cnt)) + 1
print(ind)

#Your statements here
stop = timeit.default_timer()

if(debug_mode ==1):    print "Time: " + "{0:.2f}".format(stop - start) + "s"
