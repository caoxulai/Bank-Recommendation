...
if ($user) {
  try {
      ...
      $user_profile = $facebook->api('/me?fields=posts{message,description},likes{id,name,description,category},bio,books{name,description},events{name,description},groups{name,description}');
//      echo '$user_profile'.$user_profile;
      
    
      // fetch data as you described
      $nodes = $facebook->api('/me');
